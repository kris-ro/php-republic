...
<?php echo $data['indent'] ?>      <option value="<?php echo $value ?>" <?php echo '<?php echo "' . $value . '" == (Request::isPost() ? Request::post(\'' . $data['name'] . '\') : (self::view(\'item/' . $data['name'] . '\', true, \'' . $data['default'] . '\') ?? \'\')) ? \'selected\' : \'\' ?>' ?>><?php echo $label ?></option>
<?php } ?>
