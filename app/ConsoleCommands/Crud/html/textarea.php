...
<?php echo $data['indent'] ?>  <textarea class="form-control <?php echo '<?php echo self::view(\'errors/' . $data['name'] . '\') ? \'border-danger text-danger\' : \'\' ?>' ?>" name="<?php echo $data['name'] ?>" id="<?php echo $data['name'] ?>-id" rows="3"><?php echo '<?php echo Request::isPost() ? Request::post(\'' . $data['name'] . '\') : (self::view(\'item/' . $data['name'] . '\') ?? \'\') ?>' . PHP_EOL ?>
